[32;1mINFO [0m [1m[90mgit_cliff[39m[0m > Using configuration from: cliff.toml
# Changelog
...
- Record v0.2.0-Alpha publication in release manifest

Updates release-manifest.json to reflect the completed Automated-mode
publication: tag pushed, GitHub release created with all artifacts,
release URL recorded.

Co-Authored-By: Claude Sonnet 5 <noreply@anthropic.com>

- Fix packaged app: HashRouter for file:// loading, relative icon paths

v0.2.0-Alpha was effectively unusable once actually installed (as opposed
to run from the dev server): BrowserRouter can't resolve routes under
file://, so no page ever rendered and navigation did nothing; the social
icons used hardcoded absolute paths that also only resolve against a real
server. Neither bug was visible because every test this project has run
used the Vite dev server, never a packaged build.

Switches to HashRouter (pure client-side, no server needed) and builds
icon paths from Vite's BASE_URL. Also adds renderer console forwarding to
the main process so a packaged app's JS errors aren't silently invisible
next time.

Bumps to 0.2.1-alpha.
...
- Update README for Alpha and restore electron-builder in lockfile

README now reflects that yt-dlp downloading is live rather than stubbed.
package-lock.json picks up electron-builder, which was declared in
package.json but missing from node_modules until this session's release
prep reinstalled it.

Co-Authored-By: Claude Sonnet 5 <noreply@anthropic.com>

- Add v0.2.0-Alpha release prep artifacts and audit log entry

Records the release-engineer manifest, raw git-cliff changelog, and
Discord announcement draft for v0.2.0-Alpha, plus the audit-log entry
for this session's full working-tree review.
